...
class NoteDetailViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if currentNote.title.isEmpty && currentNote.content.isEmpty {
    ...
    func saveCurrentNote() {
        ...
        if !isNoteModified && currentNote.title.isEmpty && currentNote.content.isEmpty && title.isEmpty && content.isEmpty {
            delegate?.dataUpdated(for: currentNote, deleted: true, newTitle: nil, newContent: nil)
            return
        }
        ...
        if title.isEmpty && content.isEmpty {
            ...
            delegate?.dataUpdated(for: currentNote, deleted: false, newTitle: title, newContent: content)
        }
    ...
    private func setTextViewText() {
        ...
        let titleAttributes: [NSAttributedString.Key: Any] = [
            .font: UIFont.boldSystemFont(ofSize: FontSizes.subheader) // Увеличенный размер шрифта
        ]
        ...
        let contentAttributes: [NSAttributedString.Key: Any] = [
            .font: UIFont.systemFont(ofSize: FontSizes.body)
        ]
    ...
    private func setupNavigationBar() {
        let moreActions = UIBarButtonItem(image: UIImage(systemName: UIElements.ellipsisCircle), style: .plain, target: self, action: nil)
        let shareAction = UIBarButtonItem(barButtonSystemItem: .action, target: self, action: #selector(shareAction))
        
        let menu = UIMenu(title: "", children: [
            UIAction(title: UIElements.delete, image: UIImage(systemName: UIElements.trash), attributes: .destructive, handler: { [weak self] _ in
                self?.deleteNote()
            })
        ])
    ...
    private func deleteNote() {
        ...
        DispatchQueue.main.async {
            self.delegate?.dataUpdated(for: self.currentNote, deleted: true, newTitle: nil, newContent: nil)
